...
public class GuiModifiersManager
{
    ...
    public void draw(int mouseX, int mouseY, float partialTicks)
    {
        ...
        mc.fontRenderer.drawStringWithShadow("Modifiers", this.area.x + 6, this.area.y + 7, 0xffffff);

        this.add.drawButton(mc, mouseX, mouseY, partialTicks);

        if (this.fixture == null)
        {
            this.editor.drawCenteredString(mc.fontRenderer, "Select camera fixture...", this.area.x + this.area.w / 2, this.area.y + 28, 0xcccccc);
        }
        else
        {
            for (GuiAbstractModifierPanel<AbstractModifier> panel : this.panels)
