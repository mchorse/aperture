 ...
 */
public class GuiLookFixturePanel extends GuiAbstractFixturePanel<LookFixture>
{
    public GuiPointModule point;
    public GuiTargetModule target;

    public GuiLookFixturePanel(Minecraft mc, GuiCameraEditor editor)
    {
        super(mc, editor);

        this.point = new GuiPointModule(mc, editor);
        this.target = new GuiTargetModule(mc, editor);

        this.point.resizer().parent(this.area).set(0, 10, 80, 80).x(1, -80);
        this.target.resizer().parent(this.area).set(0, 60, 100, 80);

        this.children.add(this.point, this.target);
    }
