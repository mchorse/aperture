...
public class ShaderOptionVariableConstTransformer extends ClassMethodTransformer
{
    ...
    public void processMethod(String name, MethodNode method)
    {
        ...
        while (it.hasNext())
        {
            ...
            if (node.getOpcode() == Opcodes.NEW && "net/optifine/shaders/config/ShaderOptionVariableConst".equals(((TypeInsnNode) node).desc))
            {
                ((TypeInsnNode)node).desc = "mchorse/aperture/client/AsmShaderHandler$ShaderUniformConstOption";
            }
            else if (node.getOpcode() == Opcodes.INVOKESPECIAL && "net/optifine/shaders/config/ShaderOptionVariableConst".equals(((MethodInsnNode) node).owner))
            {
                ((MethodInsnNode)node).owner = "mchorse/aperture/client/AsmShaderHandler$ShaderUniformConstOption";
            }
        }
    }
}
